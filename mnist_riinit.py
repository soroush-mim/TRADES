...
parser.add_argument('--attack-num-steps', default=40,type=int,
                    help='perturb number of steps for attack in eval')
...
parser.add_argument('--T', default=10,type=int,
                    help='number of stages for wieght re initialization')
parser.add_argument('--shrink', default=0.4, type=float
                    help='shrink size for re initialization')
parser.add_argument('--perturb', default=0.1, type=float
                    help='shrink size for re initialization')
...
def shrink_perturb(model, shrink= args.shrink, perturb=args.perturb):
...
def main():
    ...
    P = int(args.epochs / args.T) #epochs per stage

    for stage in range(args.T):
        if stage == 0:
            continue
        else:
            model = shrink_perturb(model)


        for epoch in range(1, P + 1):
            # adjust learning rate for SGD
            real_epoch = stage*P + epoch
            adjust_learning_rate(optimizer, real_epoch)
            # adversarial training
            train(args, model, device, train_loader, optimizer, real_epoch)
            ...
            if epoch % args.save_freq == 0:
                torch.save(model.state_dict(),
                        os.path.join(model_dir, 'model-nn-epoch{}.pt'.format(real_epoch)))
                torch.save(optimizer.state_dict(),
                        os.path.join(model_dir, 'opt-nn-checkpoint_epoch{}.tar'.format(real_epoch)))
